...
def toc_to_csv(toc, output_path):
    rows = []
    for line in toc.split('\n'):
        if '(p ' in line:
            title, page = line.rsplit('(p ', 1)
            page = page.rstrip(')')
            rows.append([title.strip(), page.strip()])
...
def main():
    ...
    # Save the refined TOC to a CSV file
    output_path = "refined_table_of_contents.csv"
    toc_to_csv(refined_toc, output_path)
    
    print(f"Table of contents created and saved to '{output_path}'.")
